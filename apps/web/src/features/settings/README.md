...
- **network-settings.tsx**: 导出 `InboundAccessSettingsGroup` 与 `ProxySettingsGroup`，嵌入 Network 设置页（与 Server Endpoint 合并）。Inbound access 控制 Electron-managed Cradle Server API 与 built-in relayd 是否只监听本机或接受其他设备连接，保存到 server-owned Network preferences 并在重启后生效；Proxy group 负责 outbound proxy 启用开关、来源下拉（Follow System / Custom / Environment）、条件出现的 Custom URL 与当前解析状态行。
...
- **worktree-settings.tsx**: Worktrees 设置页，集中展示 Cradle-managed git worktrees、总磁盘占用、绑定会话数，并提供单个 abandon cleanup 与全局 retention policy cleanup；策略上限写入 server-owned App preferences，实际清理由 Worktree 后端 API 执行。
- **use-app-preferences.ts**: App preferences query / mutation hook，读取与写入 server-owned feature flags 与 managed worktree cleanup 策略；`useFeatureFlag` 是前端能力入口门控的统一读取口。
...
- **use-network-preferences.ts**: Network preferences query / mutation hook，读取与写入 Cradle server Network preferences，包括 inbound listener intent 与 outbound proxy preferences；保存 outbound proxy 后刷新 proxy status 查询。
