...
Normal turn streaming is event-first. `streamTurn()` subscribes to `event.subscribe()`, sends a prompt with a Cradle-generated OpenCode `messageID`, and treats assistant `message.updated` events whose `parentID` matches that message id as the active turn. When the assistant reaches a terminal state, the adapter reads `session.message()` once to recover missed parts before emitting the final AI SDK `finish` chunk. The stream projector handles text deltas that arrive before part snapshots and avoids replaying text already emitted from SSE.
